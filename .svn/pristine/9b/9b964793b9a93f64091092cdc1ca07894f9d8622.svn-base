...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _dateLabel.font = [UIFont fontWithName:@"Arial Unicode MS" size:13.0f];
...
- (void)setModel:(FavoriteModel *)model
{
    ...
    if (_model != model) {
        ...
        _model = model;
        NSString * str = [_model.BrowseTime componentsSeparatedByString:@" "].firstObject;
        NSString * str1 = [str substringFromIndex:5];
        _dateLabel.text = str1; // 时间
        _titleLabel.text = _model.titleStr; // 标题
        [_imgView sd_setImageWithURL:[NSURL URLWithString:_model.imgUrl]]; // 图片
